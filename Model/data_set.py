import numpy as np
import os
import sentencepiece as spm
import sys
import torch


def train_model(fname, prefix):
    """
    使用SentencePiece模型训练文本数据

    Args:
    fname: str, 输入文本文件名
    prefix: str, SentencePiece模型前缀

    Returns:
    None
    """
    spm.SentencePieceTrainer.train(
        input=fname, model_prefix=prefix, vocab_size=4118, )
    # user_defined_symbols=['侠之大者', '为国为民'])


def load_file_into_splits(text_file, split_ratio):
    """
    将文本文件按照指定比例划分为训练集和测试集

    Args:
    text_file: str, 文本文件名
    split_ratio: float, 划分比例

    Returns:
    str, str: 划分后的训练集和测试集文本内容
    """
    with open(text_file, 'r', encoding='utf-8') as file:
        data = file.read()
    split_idx = int(len(data) * split_ratio)
    return data[:split_idx], data[split_idx:]


def load_tokenizer(model_file):
    """
    加载已训练的SentencePiece模型

    Args:
    model_file: str, SentencePiece模型文件名

    Returns:
    bool, SentencePieceProcessor: 加载成功标识和加载的SentencePieceProcessor对象
    """
    sp = spm.SentencePieceProcessor()
    if not sp.load(model_file=model_file):
        return False, None
    else:
        return True, sp


def encode_and_save(sp, content, prefix):
    """
    使用加载的SentencePiece模型对文本内容进行编码并保存为二进制文件

    Args:
    sp: SentencePieceProcessor, 加载的SentencePieceProcessor对象
    content: str, 待编码的文本内容
    prefix: str, 文件名前缀

    Returns:
    None
    """
    token_ids = sp.encode(content, out_type=int)
    print(f"data split of {prefix} has {len(token_ids)} tokens")
    token_ids = np.array(token_ids, dtype=np.int32)
    token_ids.tofile(os.path.join(os.path.dirname(__file__), "{}.dat".format(prefix)))


def gen_dataset(text_file, model_file):
    """
    根据文本数据和训练好的SentencePiece模型生成数据集

    Args:
    text_file: str, 文本文件名
    model_file: str, SentencePiece模型文件名

    Returns:
    None
    """
    flag, sp = load_tokenizer(model_file)
    if not flag:
        print(f"load tokenizer model from: {model_file} failed")
        sys.exit(1)

    split_ratio = 0.9
    train_text, test_text = load_file_into_splits(text_file, split_ratio)
    encode_and_save(sp, train_text, "train")
    encode_and_save(sp, test_text, "test")


def get_batch(data):
    """
    从数据集中获取一个batch的数据用于训练

    Args:
    data: numpy.ndarray, 数据集

    Returns:
    torch.Tensor, torch.Tensor: 输入数据和目标数据的tensor
    """
    batch_size = 4
    block_size = 16
    ix = torch.randint(len(data) - block_size, (batch_size,))
    x = torch.stack([torch.from_numpy((data[i:i + block_size]).astype(np.int32)) for i in ix])
    y = torch.stack([torch.from_numpy((data[i + 1:i + 1 + block_size]).astype(np.int32)) for i in ix])
    return x, y


def test_samples():
    """
    加载数据并打印样本

    Args:
    None

    Returns:
    None
    """
    train_data = np.memmap(os.path.join("./", 'train.dat'), dtype=np.int32, mode='r')
    x, y = get_batch(train_data)

    model_file = "tieba_cutted.model"
    flag, sp = load_tokenizer(model_file)
    if not flag:
        print(f"load tokenizer model from: {model_file} failed")
        sys.exit(1)

    # print(x, y)
    for features, targets in zip(x, y):
        print("feature:", sp.decode(features.tolist()))
        print("target:", sp.decode(targets.tolist()))


if __name__ == '__main__':
    train_model("tieba_cutted.txt", "tieba_cutted")
    gen_dataset("tieba_cutted.txt", "tieba_cutted.model")
    # test_samples()
